 ...
 * @package App
 */
class Furdeco extends Courier implements CourierInterface
{
    /**
     ...
     * @var string
     */
    private static $batchFilesDirectory = "/couriers/Furdeco/";
    ...
    public function sendDispatchData()
    {
        try {
            ...
            if($csvFile = self::getBatchFile()) {
                ...
                // Send batch file as an email with file attachment
                $dt = new \DateTime();
                $this->sendEmail("Dispatch data for ".$dt->format('d/m/Y'), "Please find the attached batch file for today's consignments", $csvFile);
    ...
    /**
     * Send dispatch data file
     * @param string $subject
     * @param string $message
     * @param string $file_path
     */
    private function sendEmail(string $subject, string $message, string $file_path)
    {
        $details = [
            'title' => 'Dispatch Data',
            'subject' => $subject,
